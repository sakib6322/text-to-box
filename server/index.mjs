...
/** Concept detail / question stem+explanation / exam title vectors — unused in UI; keep key_points only. */
const EMBED_UNUSED_CONTENT =
  String(process.env.EMBED_UNUSED_CONTENT ?? "false").toLowerCase() === "true";
...
app.get("/api/debug/embeddings-stats", async (_req, res) => {
  try {
    ...
    return res.json({
      ...
      note:
        "Only key_points.embedding is generated by default. Concept detail / question / exam vectors require EMBED_UNUSED_CONTENT=true. Matching uses key_points via /api/match-key-points.",
      embed_unused_content: EMBED_UNUSED_CONTENT,
    });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ error: e instanceof Error ? e.message : "Unknown error" });
...
app.patch("/api/concepts/:id", async (req, res) => {
  try {
    ...
    return res.json({
      concept,
      detail_embedding_saved: detailUpdated && EMBED_UNUSED_CONTENT,
    });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ error: e instanceof Error ? e.message : "Unknown error" });
...
app.post("/api/save-concept", async (req, res) => {
  try {
    ...
    const detailEmbedding =
      EMBED_UNUSED_CONTENT && detailEmbedText ? await embedTextRotating(db, detailEmbedText) : null;
    const storyHtml =
      ...
      await Promise.all(
        list
          ...
          .map(async (q) => {
            ...
            const emb = EMBED_UNUSED_CONTENT ? await embedTextRotating(db, stem ?? "") : null;
            const explanationEmbedding = EMBED_UNUSED_CONTENT
              ? await embedExplanationRotating(db, q.questionMode, mcq, sba)
              : null;
    ...
    if (payload) {
      ...
      if (mode) {
        if (EMBED_UNUSED_CONTENT) {
          ...
          patch.explanation_embedding = null;
        }
      }
...
app.post("/api/exams", async (req, res) => {
  try {
    ...
    const titleEmbedding =
      EMBED_UNUSED_CONTENT && embedText ? toPgVector(await embedTextRotating(db, embedText)) : null;
